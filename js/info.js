function updateInfoPanel() {
    ...
    text += `--- Transformações ---\n`;
    text += `Translação: (${selectedObj.translation[0].toFixed(3)}, ${selectedObj.translation[1].toFixed(3)})\n`;
    text += `Rotação: ${selectedObj.rotation.toFixed(2)} graus\n`;
    text += `Escala: (${selectedObj.scale[0].toFixed(2)}, ${selectedObj.scale[1].toFixed(2)})\n\n`;

    text += `Centro (world-space): (${center[0].toFixed(3)}, ${center[1].toFixed(3)})\n\n`;
    
    text += `--- Vértices (local-space) ---\n`;
    selectedObj.vertices.forEach((v, i) => {
        text += `v${i}: (${v[0].toFixed(3)}, ${v[1].toFixed(3)})\n`;
    });

    text += `\n--- Vértices (world-space) ---\n`;
    selectedObj.vertices.forEach((v, i) => {
        let w = applyMat4ToVec2(selectedObj.transform, v);
        text += `v${i}: (${w[0].toFixed(3)}, ${w[1].toFixed(3)})\n`;
    });
